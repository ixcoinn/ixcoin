...
The IXC daemon can be configured to act as a trusted "border
router", implementing the ixc wire protocol and relay, making
...
The ZeroMQ feature in IXC requires ZeroMQ API version 4.3.2 or
...
during the *configure* step of building ixcoind:
...
For instance:
    ...
    $ ixcoind -zmqpubhashtx=tcp://127.0.0.1:29882 \
               -zmqpubrawtx=ipc:///tmp/ixcoind.tx.raw

These options can also be provided in ixc.conf.
...
From the perspective of ixcoind, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into IXC directly. Furthermore, no information is
...
using. IXC appends an up-counting sequence number to each
