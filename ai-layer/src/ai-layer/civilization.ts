  ...
  export async function runCycle(): Promise<{ cycleId: number; actionsProcessed: number; totalReward: number } | null> {
    ...
    try {
      ...
      for (const entity of entities) {
        ...
        const safeReward = Math.min(decision.rewardIssued, poolRemaining * 0.001);
        ...
        await db.insert(aiActionsTable).values({ aiId: entity.aiId, actionType: decision.actionType, decision: decision.decision, rewardIssued: safeReward, energyCost: decision.energyCost, expGained: decision.expGained, networkActivity, poolRemaining, outcome: decision.outcome, metadata: decision.metadata });
