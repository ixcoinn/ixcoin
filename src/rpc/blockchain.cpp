...
static UniValue SoftForkMajorityDesc(int version, CBlockIndex *pindex,
    ...
    switch (version) {
        case 2:
            activated = pindex->GetHeight() >= consensusParams.BitcoinSoftForksHeight;
            break;
        case 3:
            activated = pindex->GetHeight() >= consensusParams.BitcoinSoftForksHeight;
            break;
...
static UniValue getvestinginfo(const Config &config, const JSONRPCRequest &request) {
    if (request.fHelp || request.params.size() != 0) {
        throw std::runtime_error(
            "getvestinginfo\n"
            "\nReturns vesting schedule information for the first block reward.\n"
            "\nResult:\n"
            "{\n"
            "  \"total_vesting_amount\": xxxxx,  (numeric) Total vesting amount in IXC\n"
            "  \"vesting_periods\": xx,           (numeric) Number of vesting periods\n"
            "  \"blocks_per_period\": xxxx,       (numeric) Blocks per vesting period\n"
            "  \"current_height\": xxxx,          (numeric) Current block height\n"
            "  \"total_vested\": xxxxx,           (numeric) Total vested so far in IXC\n"
            "  \"total_locked\": xxxxx,           (numeric) Total still locked in IXC\n"
            "  \"tranches\": [                    (array) Vesting tranches detail\n"
            "    {\n"
            "      \"tranche\": x,               (numeric) Tranche number\n"
            "      \"amount_ixc\": xxxxx,         (numeric) Amount in IXC\n"
            "      \"unlock_block\": xxxx,        (numeric) Block height when unlocked\n"
            "      \"status\": \"locked|unlocked\" (string) Current status\n"
            "    }\n"
            "  ]\n"
            "}\n");
    }
    ...
    UniValue result(UniValue::VOBJ);
    result.pushKV("total_vesting_amount", ValueFromAmount(Amount(VESTING_TOTAL_AMOUNT)));
    result.pushKV("spendable_immediately", ValueFromAmount(Amount(400000000000000)));
    result.pushKV("vesting_periods", VESTING_NUM_PERIODS);
    result.pushKV("blocks_per_period", VESTING_PERIOD_BLOCKS);
    result.pushKV("current_height", currentHeight);

    uint64_t totalVested = GetVestedAmount(currentHeight);
    result.pushKV("total_vested", ValueFromAmount(Amount(totalVested)));
    result.pushKV("total_locked", ValueFromAmount(Amount(VESTING_TOTAL_AMOUNT - totalVested)));

    UniValue tranches(UniValue::VARR);
    uint64_t perPeriod = 41666666666667;
    uint64_t lastPeriod = 41666666666663;
    for (int i = 0; i < VESTING_NUM_PERIODS; i++) {
        UniValue tranche(UniValue::VOBJ);
        tranche.pushKV("tranche", i + 1);
        uint64_t amount = (i == VESTING_NUM_PERIODS - 1) ? lastPeriod : perPeriod;
        tranche.pushKV("amount_ixc", ValueFromAmount(Amount(amount)));
        int unlockBlock = (i + 1) * VESTING_PERIOD_BLOCKS;
        tranche.pushKV("unlock_block", unlockBlock);
        tranche.pushKV("status", (currentHeight >= unlockBlock) ? "unlocked" : "locked");
        tranches.push_back(tranche);
    }
    result.pushKV("tranches", tranches);
...
static const CRPCCommand commands[] = {
    ...
    //  ------------------- ------------------------  ----------------------  ------ ----------
    { "blockchain",         "getvestinginfo",         getvestinginfo,         true,  {} },
