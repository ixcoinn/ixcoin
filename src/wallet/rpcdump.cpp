...
UniValue importprivkey(const Config &config, const JSONRPCRequest &request) {
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 3)
        throw std::runtime_error(
            "importprivkey \"ixcprivkey\" ( \"label\" ) ( rescan )\n"
            "\nAdds a private key (as returned by dumpprivkey) to your "
            "wallet.\n"
            "\nArguments:\n"
            "1. \"ixcprivkey\"   (string, required) The private key (see "
            "dumpprivkey)\n"
...
UniValue dumpprivkey(const Config &config, const JSONRPCRequest &request) {
    ...
    if (request.fHelp || request.params.size() != 1) {
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"   (string, required) The ixc address for the "
            "private key\n"
            "\nResult:\n"
    ...
    return CIXCSecret(vchSecret).ToString();
}
...
UniValue dumpwallet(const Config &config, const JSONRPCRequest &request) {
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "(either absolute or relative to ixcoind)\n"
            "\nResult:\n"
            "{                           (json object)\n"
     ...
     * https://github.com/ixc/ixc/issues/9934
    ...
    // produce output
    file << strprintf("# Wallet dump created by IXC %s\n", CLIENT_BUILD);
    ...
    for (std::vector<std::pair<int64_t, CKeyID>>::const_iterator it =
        ...
        if (pwallet->GetKey(keyid, key)) {
            file << strprintf("%s %s ", CIXCSecret(key).ToString(),
                              strTime);
