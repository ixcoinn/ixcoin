...
static UniValue getnewaddress(const Config &config,
    ...
    if (request.fHelp || request.params.size() > 1) {
        throw std::runtime_error(
            ...
            "\nReturns a new IXC address for receiving payments.\n"
            ...
            "the given name.\n"
            "\nResult:\n"
            "\"address\"    (string) The new ixc address\n"
            "\nExamples:\n" +
...
static UniValue getaccountaddress(const Config &config,
    ...
    if (request.fHelp || request.params.size() != 1) {
        throw std::runtime_error(
            ...
            "\nDEPRECATED. Returns the current IXC address for receiving "
            ...
            "the given name.\n"
            "\nResult:\n"
            "\"address\"          (string) The account ixc address\n"
            "\nExamples:\n" +
...
static UniValue getrawchangeaddress(const Config &config,
    ...
    if (request.fHelp || request.params.size() > 1) {
        throw std::runtime_error(
            "getrawchangeaddress\n"
            "\nReturns a new IXC address, for receiving change.\n"
...
static UniValue setaccount(const Config &config,
    ...
    if (request.fHelp || request.params.size() < 1 ||
        ...
        throw std::runtime_error(
            ...
            "address.\n"
            "\nArguments:\n"
            "1. \"address\"         (string, required) The ixc address to "
...
static UniValue getaccount(const Config &config,
    ...
    if (request.fHelp || request.params.size() != 1) {
        throw std::runtime_error(
            ...
            "address.\n"
            "\nArguments:\n"
            "1. \"address\"         (string, required) The ixc address for "
            "account lookup.\n"
            "\nResult:\n"
...
static UniValue getaddressesbyaccount(const Config &config,
    ...
    if (request.fHelp || request.params.size() != 1) {
        throw std::runtime_error(
            ...
            "  \"address\"         (string) a ixc address associated with "
            "the given account\n"
            "  ,...\n"
            "]\n"
            "\nExamples:\n" +
...
static UniValue sendtoaddress(const Config &config,
    ...
    if (request.fHelp || request.params.size() < 2 ||
        ...
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"            (string, required) The ixc address "
            "to send to.\n"
            ...
            "mvcs than you enter in the amount field.\n"
            "\nResult:\n"
...
static UniValue listaddressgroupings(const Config &config,
    ...
    if (request.fHelp) {
        throw std::runtime_error(
            ...
            "  [\n"
            "    [\n"
            "      \"address\",            (string) The ixc address\n"
...
static UniValue sendfrom(const Config &config, const JSONRPCRequest &request) {
    ...
    if (request.fHelp || request.params.size() < 3 ||
        ...
        throw std::runtime_error(
            ...
            "reflect\n"
            "                       the spend.\n"
            "2. \"toaddress\"         (string, required) The ixc address "
            "to send funds to.\n"
...
static UniValue sendmany(const Config &config, const JSONRPCRequest &request) {
    ...
    if (request.fHelp || request.params.size() < 2 ||
        ...
        throw std::runtime_error(
            ...
            "    {\n"
            "      \"address\":amount   (numeric or string) The ixc "
            ...
            "mvcs than you enter in their corresponding amount field.\n"
    ...
    for (const std::string &name_ : keys) {
        ...
        if (!IsValidDestination(dest)) {
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY,
                               std::string("Invalid IXC address: ") +
                                   name_);
        }
...
static UniValue addmultisigaddress(const Config &config,
    ...
    if (request.fHelp || request.params.size() < 2 ||
        ...
        std::string msg =
            ...
            "Each key is a IXC address or hex-encoded public key.\n"
            ...
            "2. \"keys\"         (string, required) A json array of ixc "
            "addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"address\"  (string) ixc address or hex-encoded "
            "public key\n"
            "       ...,\n"
            "     ]\n"
...
static UniValue listtransactions(const Config &config,
    ...
    if (request.fHelp || request.params.size() > 4) {
        throw std::runtime_error(
            ...
            "    \"address\":\"address\",    (string) The ixc address of "
...
static UniValue gettransaction(const Config &config,
    ...
    if (request.fHelp || request.params.size() < 1 ||
        ...
        throw std::runtime_error(
            ...
            "default account.\n"
            "      \"address\" : \"address\",          (string) The ixc "
...
static UniValue walletpassphrase(const Config &config,
    ...
    if (pwallet->IsCrypted() && (request.fHelp || request.params.size() != 2)) {
        throw std::runtime_error(
            ...
            "private keys such as sending mvcs\n"
            "\nArguments:\n"
...
static UniValue encryptwallet(const Config &config,
    ...
    StartShutdown();
    return "wallet encrypted; IXC server stopping, restart to run with "
...
static UniValue listunspent(const Config &config,
    ...
    if (request.fHelp || request.params.size() > 4) {
        throw std::runtime_error(
            ...
            "3. \"addresses\"    (string) A json array of ixc addresses to "
            "filter\n"
            "    [\n"
            ...
            "    \"vout\" : n,               (numeric) the vout value\n"
            "    \"address\" : \"address\",    (string) the ixc address\n"
...
static UniValue fundrawtransaction(const Config &config,
    ...
    if (request.fHelp || request.params.size() < 1 ||
        ...
        throw std::runtime_error(
            ...
            "address) The ixc address to receive the change\n"
            ...
            if (options.exists("changeAddress")) {
                ...
                if (!IsValidDestination(dest)) {
                    throw JSONRPCError(
                        ...
                        "changeAddress must be a valid ixc address");
                }
