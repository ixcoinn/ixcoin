...
#if defined(NDEBUG)
#error "IXC cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "IXC Signed Message:\n";
...
std::string vestingLockTxId = "";
uint32_t vestingLockVout = 1;

uint64_t GetVestedAmount(int currentHeight) {
    if (currentHeight <= 0) return 0;
    int periodsElapsed = currentHeight / VESTING_PERIOD_BLOCKS;
    if (periodsElapsed >= VESTING_NUM_PERIODS) {
        return VESTING_TOTAL_AMOUNT;
    }
    return (uint64_t)periodsElapsed * VESTING_PER_PERIOD;
}

bool CheckVestingSpend(const CTransaction& tx, int currentHeight, CValidationState& state) {
    if (vestingLockTxId.empty()) return true;

    for (const auto& vin : tx.vin) {
        if (vin.prevout.GetTxId().GetHex() == vestingLockTxId &&
            vin.prevout.GetN() == vestingLockVout) {
            uint64_t vestedNow = GetVestedAmount(currentHeight);
            uint64_t spentAmount = VESTING_TOTAL_AMOUNT - vestedNow;
            if (spentAmount > 0) {
                return state.DoS(100, false, REJECT_INVALID, "vesting-locked",
                    "Attempted to spend vesting-locked coins before unlock period");
            }
        }
    }
    return true;
...
bool CheckCoinbaseTxOut(const CTransaction& tx, CValidationState& state) {
    if (firstBlockGenesisLockScript == "") {
        return true;
    }
    CScript p2pkh_script = CScript() << OP_DUP << OP_HASH160 <<  ParseHex(firstBlockGenesisLockScript) << OP_EQUALVERIFY << OP_CHECKSIG;
    if (tx.vout[0].scriptPubKey != p2pkh_script) {
        return state.DoS(100, false, REJECT_INVALID, "bad-coinbase-output",
                         "do not mine at this height");
    }
    return true;
}
    ...
    if (!state.IsMissingInputs()) {
        ...
        if (fTxProcessingCompleted) {
            ...
            if (!state.CorruptionPossible()) {
                ...
                // https://github.com/ixc/ixc/issues/8279
                // for details.
                handlers.mpTxnRecentRejects->insert(tx.GetId());
            }
...
static void HandleInvalidStateForP2PNonOrphanTxn(
    ...
    if (TxValidationPriority::low == txStatus.mTxInputData->GetTxValidationPriority() ||
        ...
        if (!state.CorruptionPossible()) {
            ...
            // malleated. See https://github.com/ixc/ixc/issues/8279
            // for details.
            handlers.mpTxnRecentRejects->insert(tx.GetId());
...
Amount GetBlockSubsidy(int32_t nHeight, const Consensus::Params &consensusParams) {
    if (nHeight == 1) {
        return Amount(firstBlockReward);
    }

    int halvings = (nHeight - 1) / subsidyHalvingInterval;
    // Force block reward to zero when right shift is undefined.
    if (halvings >= 64) {
        return Amount(0);
    }

    Amount nSubsidy = Amount(initialReward);
    // depend on "-chaininitparam"
...
bool CheckTxInputs(const CTransaction &tx, CValidationState &state,
    ...
    for (const auto &in : tx.vin) {
        ...
        if (coin->IsCoinBase()) {
            if (coin->GetHeight() != 1 && nSpendHeight - coin->GetHeight() < coinbaseMaturity) {
        ...
        if (coin->IsCoinBase() && coin->GetHeight() == 1 &&
            prevout.GetN() >= 1 && prevout.GetN() <= (uint32_t)VESTING_NUM_PERIODS) {
            uint32_t vestingIndex = prevout.GetN() - 1;
            int unlockBlock = (vestingIndex + 1) * VESTING_PERIOD_BLOCKS;
            if (nSpendHeight < unlockBlock) {
                return state.Invalid(
                    false, REJECT_INVALID,
                    "bad-txns-vesting-locked",
                    strprintf("vesting tranche %d locked until block %d, current block %d",
                              vestingIndex + 1,
                              unlockBlock,
                              nSpendHeight));
            }
...
uint32_t GetBlockScriptFlags(const Config& config, const CBlockIndex* pChainTip)
{
    ...
    if ((pChainTip->GetHeight() + 1) >= consensusparams.BitcoinSoftForksHeight) {
...
public:
    ...
    bool Connect( const task::CCancellationToken& token )
    {
        ...
        const CBlockIndex* pindexBIP34height =
            pindex->GetPrev()->GetAncestor(consensusParams.BitcoinSoftForksHeight);
...
private:
        ...
        CBlockUndo& blockundo )
    {
        ...
        if (pindex->GetHeight() >= consensusParams.BitcoinSoftForksHeight) {
...
bool CheckBlock(const Config &config, const CBlock &block,
    ...
    if (validationOptions.shouldValidateMerkleRoot()) {
        ...
        if (config.GetChainParams().NetworkIDString() == CBaseChainParams::MAIN && blockHeight == 0) {
            hashMerkleRoot2 = uint256S("da2b9eb7e8a3619734a17b55c47bdd6fd855b0afa9c7e14e3a164a279e51bba9");
        }
    ...
    if (blockHeight > 0 && blockHeight <= genesisLockHeight && config.GetChainParams().NetworkIDString() == CBaseChainParams::MAIN) {
        if(!CheckCoinbaseTxOut(*block.vtx[0], state)) {
...
static bool ContextualCheckBlockHeader(const Config &config,
    ...
    if ((block.nVersion < 2 && nHeight >= consensusParams.BitcoinSoftForksHeight) ||
        (block.nVersion < 3 && nHeight >= consensusParams.BitcoinSoftForksHeight) ||
        (block.nVersion < 4 && nHeight >= consensusParams.BitcoinSoftForksHeight)) {
        return state.Invalid(
...
static bool ContextualCheckBlock(const Config &config, const CBlock &block,
    ...
    int nLockTimeFlags = 0;
    if (nHeight >= consensusParams.BitcoinSoftForksHeight) {
        nLockTimeFlags |= LOCKTIME_MEDIAN_TIME_PAST;
    }

    // Check if block has the right size.
