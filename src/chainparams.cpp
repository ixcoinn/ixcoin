...
static CBlock CreateGenesisBlock(const char *pszTimestamp,
    ...
    Amount spendableAmount = Amount(400000000000000);
    Amount vestingPerPeriod = Amount(41666666666667);
    Amount vestingLastPeriod = Amount(41666666666663);
    int vestingOutputs = 12;

    if (genesisReward == Amount(900000000000000) && !isTestNet) {
        txNew.vout.resize(1 + vestingOutputs);
        txNew.vout[0].nValue = spendableAmount;
        txNew.vout[0].scriptPubKey = genesisOutputScript;
        for (int i = 0; i < vestingOutputs; i++) {
            if (i == vestingOutputs - 1) {
                txNew.vout[1 + i].nValue = vestingLastPeriod;
            } else {
                txNew.vout[1 + i].nValue = vestingPerPeriod;
            }
            txNew.vout[1 + i].scriptPubKey = genesisOutputScript;
        }
    } else {
        txNew.vout.resize(1);
...
static CBlock CreateGenesisBlock(uint32_t nTime, uint32_t nNonce,
    ...
    const CScript genesisOutputScript =
        CScript() << OP_DUP << OP_HASH160
                  << ParseHex("e05917944e509873475a343dd4e76a45fa6e76ce")
...
public:
    CMainParams() {
        ...
        vSeeds.push_back(CDNSSeedData("seed1.ixcoin.com", "seed1.ixcoin.com"));
        vSeeds.push_back(CDNSSeedData("seed2.ixcoin.com", "seed2.ixcoin.com"));
        ...
        chainTxData = ChainTxData{
            ...
            // (the tx=... number in the SetBestChain ixcoind.log lines)
            5545024,
