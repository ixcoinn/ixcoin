...
const char *const IXC_CONF_FILENAME = "ixc.conf";
const char *const IXC_PID_FILENAME = "ixcoind.pid";
...
// Windows < Vista: C:\Documents and Settings\Username\Application Data\IXC
// Windows >= Vista: C:\Users\Username\AppData\Roaming\IXC
// Mac: ~/Library/Application Support/IXC
// Unix: ~/.ixc
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "IXC";
#else
    fs::path pathRet;
...
#ifdef __APPLE__
    // Mac
    return pathRet / "Library/Application Support/IXC";
#else
    // Unix
    return pathRet / ".ixc";
#endif
...
#ifndef WIN32
fs::path GetPidFile() {
    fs::path pathPidFile(gArgs.GetArg("-pid", IXC_PID_FILENAME));
