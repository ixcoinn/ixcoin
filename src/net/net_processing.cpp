                                      ...
                                      const CNetMsgMaker& msgMaker)
{
    ...
    try
    {
        ...
        if(!ValidateForkHeight(msg, SAFE_MODE_DEFAULT_MAX_FORK_DISTANCE))
        {
            Misbehaving(pfrom, misbehaviour_penalty,
...
bool DetectStalling(const Config& config, const CNodePtr& pto, const CNodeStatePtr& state)
{
    ...
    if (state->vBlocksInFlight.size() > 0) {
        ...
        auto timeoutPeers = DEFAULT_BLOCK_DOWNLOAD_TIMEOUT_PER_PEER * nOtherPeersWithValidatedDownloads;
