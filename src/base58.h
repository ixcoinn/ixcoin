 ...
 */
class CIXCSecret : public CBase58Data {
public:
    ...
    CIXCSecret(const CKey &vchSecret) { SetKey(vchSecret); }
    CIXCSecret() {}
};

template <typename K, int Size, CChainParams::Base58Type Type>
class CIXCExtKeyBase : public CBase58Data {
public:
    ...
    CIXCExtKeyBase(const K &key) { SetKey(key); }

    CIXCExtKeyBase(const std::string &strBase58c) {
...
typedef CIXCExtKeyBase<CExtKey, BIP32_EXTKEY_SIZE,
                           CChainParams::EXT_SECRET_KEY>
    CIXCExtKey;
typedef CIXCExtKeyBase<CExtPubKey, BIP32_EXTKEY_SIZE,
