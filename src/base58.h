 ...
 */
class CMVCSecret : public CBase58Data {
public:
    ...
    CMVCSecret(const CKey &vchSecret) { SetKey(vchSecret); }
    CMVCSecret() {}
};

template <typename K, int Size, CChainParams::Base58Type Type>
class CMVCExtKeyBase : public CBase58Data {
public:
    ...
    CMVCExtKeyBase(const K &key) { SetKey(key); }

    CMVCExtKeyBase(const std::string &strBase58c) {
...
typedef CMVCExtKeyBase<CExtKey, BIP32_EXTKEY_SIZE,
                           CChainParams::EXT_SECRET_KEY>
    CMVCExtKey;
typedef CMVCExtKeyBase<CExtPubKey, BIP32_EXTKEY_SIZE,
