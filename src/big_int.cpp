...
void ixc::bint::empty_bn_deleter::operator()(bignum_st* p) const
{
    ::BN_free(p);
}

ixc::bint::bint() : value_{nullptr} {}

ixc::bint::bint(const int i) : value_(BN_new(), empty_bn_deleter())
{
    // assert(value_);
    if(!value_)
        throw big_int_error();
...
ixc::bint::bint(const int64_t i) : value_(BN_new(), empty_bn_deleter())
{
    // assert(value_);
    if(!value_)
        throw big_int_error();
...
ixc::bint::bint(const size_t i) : value_(BN_new(), empty_bn_deleter())
{
    assert(value_);
    if(!value_)
        throw big_int_error();
...
ixc::bint::bint(const std::string& n) : value_(BN_new(), empty_bn_deleter())
{
    // assert(value_);
    if(!value_)
        throw big_int_error();
...
ixc::bint::bint(const bint& other) : value_(BN_new(), empty_bn_deleter())
{
...
ixc::bint& ixc::bint::operator=(const bint& other)
{
    bint temp{other};
    swap(temp);
    return *this;
...
void ixc::bint::swap(bint& other) noexcept
{
    // assert(value_);
    using std::swap;
...
bool ixc::operator<(const bint& a, const bint& b)
{
...
ixc::bint& ixc::bint::operator<<=(const int n)
{
    if(n <= 0)
        return *this;
...
ixc::bint ixc::bint::operator-() const
{
    bint rv{*this};
    rv.negate();
    return rv;
...
int ixc::bint::spaceship_operator(
...
void ixc::bint::mask_bits(const int n)
{
...
int ixc::bint::size_bits() const { return BN_num_bits(value_.get()); }

int ixc::bint::size_bytes() const { return BN_num_bytes(value_.get()); }

ixc::bint::buffer_type ixc::bint::to_bin() const
{
    // assert(value_);
...
std::ostream& ixc::operator<<(std::ostream& os, const bint& n)
{
    if(n.value_ == nullptr)
        return os;
...
bool ixc::is_negative(const bint& n)
{
    const auto s{BN_is_negative(n.value_.get())};
    return s == 1;
}

ixc::bint ixc::abs(const bint& n) { return is_negative(n) ? -n : n; }

std::string ixc::to_string(const bint& n) // used in gdb pretty-printer
{
...
std::size_t ixc::to_size_t_limited(const bint& n)
{
    // Precondition:
    // Windows/MSVC (sizeof(long) == 4 bytes)
    // n <= numeric_limit<int32_t>::max() and n>=0
...
std::vector<uint8_t> ixc::bint::serialize() const
{
...
ixc::bint ixc::bint::deserialize(ixc::span<const uint8_t> s)
{
