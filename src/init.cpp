...
std::string HelpMessage(HelpMessageMode mode, const Config& config) {
    ...
    strUsage += HelpMessageOpt(
        "-pid=<file>",
        strprintf(_("Specify pid file (default: %s)"), IXC_PID_FILENAME));
#endif
    ...
    strUsage += HelpMessageOpt(
        "-printtoconsole",
        _("Send trace/debug info to console instead of ixcoind.log file"));
    strUsage += HelpMessageOpt("-shrinkdebugfile",
                               _("Shrink ixcoind.log file on client startup "
...
std::string LicenseInfo() {
    const std::string URL_SOURCE_CODE =
        "<https://github.com/ixc-labs/ixcoin>";
    const std::string URL_WEBSITE = "<https://www.ixcoin.com>";

    return CopyrightHolders(
               strprintf(_("Copyright (C) %i-%i"), 2021, COPYRIGHT_YEAR) +
               " ") +
           "\n" + "\n" +
...
/** Sanity checks
 *  Ensure that IXC is running in a usable environment with all
...
bool AppInitParameterInteraction(ConfigInit &config) {
    ...
    if (!gArgs.IsArgSet("-chaininitparam")) {
        ...
        if (gArgs.IsArgSet("-firstBlockReward")) {
            ...
            if (firstBlockReward > 1200000000000000) {
    ...
    if (std::string err; !config.SetMaxBlockSize(uint64_t(10000000000), &err)) {
        return InitError(err);
    }
    ...
    // Signal IXC Cash support.
    // TODO: remove some time after the hardfork when no longer needed
    // to differentiate the network nodes.
    nLocalServices = ServiceFlags(nLocalServices | NODE_IXC_CASH);
...
static bool LockDataDirectory(bool probeOnly) {
    ...
    // Make sure only a single IXC process is using the data directory.
...
bool AppInitMain(ConfigInit &config, boost::thread_group &threadGroup,
    ...
    if (gArgs.GetBoolArg("-shrinkdebugfile", default_shrinkdebugfile)) {
        // Do this first since it both loads a bunch of ixcoind.log into memory,
        // and because this needs to happen before any other ixcoind.log printing.
    ...
    LogPrintf(
        "Using config file %s\n",
        GetConfigFile(gArgs.GetArg("-conf", IXC_CONF_FILENAME)).string());
