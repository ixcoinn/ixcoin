...
#define IXC_TIMEDATA_MAX_SAMPLES 200

void AddTimeData(const CNetAddr &ip, int64_t nOffsetSample) {
    LOCK(cs_nTimeOffset);
    // Ignore duplicates
    static std::set<CNetAddr> setKnown;
    if (setKnown.size() == IXC_TIMEDATA_MAX_SAMPLES) return;
    if (!setKnown.insert(ip).second) return;

    // Add data
    static CMedianFilter<int64_t> vTimeOffsets(IXC_TIMEDATA_MAX_SAMPLES, 0);
