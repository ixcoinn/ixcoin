...
/** A hasher class for IXC's 256-bit hash (double SHA-256). */
class CHash256 {
private:
    CSHA256 sha;
...
public:
    ...
    void SingleFinalize(uint8_t hash[OUTPUT_SIZE]) {sha.Finalize(hash);}
...
/** A hasher class for IXC's 160-bit hash (SHA-256 + RIPEMD-160). */
class CHash160 {
private:
    CSHA256 sha;
...
uint256 TxSerializeHash(const T &obj, int nType = SER_GETHASH,
    ...
    if ( obj.nVersion >= 10 ) {
        ...
        ss_root << ss_in.GetSingleHash();
        ss_root << ss_in_unlock.GetSingleHash();
