...
#ifndef IXC_PROTOCOL_H
#define IXC_PROTOCOL_H
...
/**
 * IXC protocol message types. When adding new message types, don't forget
...
enum ServiceFlags : uint64_t {
    ...
    // It is currently set by all IXCOIN nodes, and is unset by SPV clients
    ...
    // protocol request. IXCOIN does not support this but a patch set
    // called IXC XT does. See BIP 64 for details on how this is
    // implemented.
    NODE_GETUTXO = (1 << 1),
    // NODE_BLOOM means the node is capable and willing to handle bloom-filtered
    // connections. IXCOIN nodes used to support this by default, without
    ...
    NODE_XTHIN = (1 << 4),
    // NODE_IXC_CASH means the node supports IXC and the
    // associated consensus rule changes.
    // This service bit is intended to be used prior until some time after the
    // UAHF activation when the IXC network has adequately separated.
    // TODO: remove (free up) the NODE_IXC_CASH service bit once no longer
    // needed.
    NODE_IXC_CASH = (1 << 5),

    // Bits 24-31 are reserved for temporary experiments. Just pick a bit that
    // isn't getting used, or one not being used much, and notify the
    // ixc-development mailing list. Remember that service bits are just
