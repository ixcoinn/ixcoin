# New chain creation
This document describes how to create a new chain from IXC.
...
3. `nDefaultPort` - Hardcoded as 9883,which can be configured in ixc.conf
4. `nMinimumChainWork` - Change the hardcode value or configured in ixc.conf
5. `checkpointData` 
6. `ChainTxData`
...
2. `asertActivationTime` and `asertAnchorParams`
    IXC mainnet use chainwork-144 as difficulty adjustment algorithm (DAA) before height 21256, then the DAA changed to 
    aserti3-2d .
